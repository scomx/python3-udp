...
but over UDP protocol instead of HTTP.
...
class WXParser:
	def __init__(self):
		...
		wxdata = { 'stationID':'unknownID', 'baro':0, 'temp':0, 'dewpoint':0, 'intemp':0, 'wctemp':0, 'rhum':0, 'inrhum':0, 'wdir':0, 'wgustdir':0, 'wspeed':0,
			'wgustspeed':0, 'precip':0, 'precip_day':0, 'precip_month':0, 'UV':0, 'solar':0, 'soiltemp':0, 'soilmoisture':0, 'leafwetness':0, 
	...
	def store_data(self, qdic):
		...
		if 'updateweatherstation.php' in qdic['data']: #Parse Weather Underground protocol
			...
			#Get air pressure
			if 'absbaromin' in qdic: wxdata.update({ 'baro': uc.inHg_hPa(qdic['absbaromin']) })
			
			#Get/convert temperatures
			if 'tempf' in qdic: wxdata.update({ 'temp': uc.c_f(qdic['tempf']) })
			if 'dewptf' in qdic: wxdata.update({ 'dewpoint': uc.c_f(qdic['dewptf']) })
			if 'indoortempf' in qdic: wxdata.update({ 'intemp': uc.c_f(qdic['indoortempf']) })
			if 'windchillf' in qdic: wxdata.update({ 'wctemp': uc.c_f(qdic['windchillf']) })
			...
			#Get/convert wind and windspeed
			if 'winddir' in qdic: wxdata.update({ 'wdir': float(qdic['winddir']) })
			...
			#Other data
			if 'soiltempf' in qdic: wxdata.update({ 'soiltemp': uc.c_f(qdic['soiltempf']) })
...
class DBStorage:
	def db_store(self, wxdict):
		...
		zOffset = (now-datetime.utcnow()).total_seconds()/60.0
	...
	def db_create(self, dbname):
		...
		try:
			...
			if conn is not None:
				...
				RES1 real, RES2 real, RES3 real, RES4 real, RES5 text);")
